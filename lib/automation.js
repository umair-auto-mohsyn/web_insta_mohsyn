...
async function bookAppointment(data) {
    ...
    try {
        ...
        // Parallel navigation after click to save 5-10 seconds
        await Promise.all([
            page.click('#signInBtn'),
            page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 20000 })
        ]);

        logToFile('Fast-Lane: Adaptive Navigation...');
        try {
            await page.goto('https://app.instacare.pk/appointments', { waitUntil: 'domcontentloaded', timeout: 8000 });
        } catch (e) {
            logToFile('Direct nav slow, waiting for trigger on current page...');
        }

        logToFile('Finding Appointment Trigger...');
        const triggerSelector = '.open-appointment-portal, button:has-text("Add Appointment"), a[href*="/appointments/Add"]';
        let trigger = await page.waitForSelector(triggerSelector, { timeout: 5000 }).catch(() => null);

        if (!trigger) {
            logToFile('Trigger not found via direct nav. Falling back to Sidebar Click...');
            try {
                await page.click('a[href="/appointments"]');
                trigger = await page.waitForSelector(triggerSelector, { timeout: 15000 });
            } catch (err) {
                logToFile(`Sidebar click failed. Current URL: ${page.url()}`);
                throw new Error('Could not find Appointment trigger after adaptive navigation.');
            }
        }
