...
async function bookAppointment(data) {
    ...
    try {
        ...
        logToFile('Fast-Lane: Direct Navigation to Appointments...');
        // Use 'load' for stability in direct navigation
        await page.goto('https://app.instacare.pk/appointments', { waitUntil: 'load', timeout: 30000 });

        logToFile('Opening Appointment Modal...');
        // Try multiple selectors for the trigger to avoid V9 timeout
        const triggerSelector = '.open-appointment-portal, button:has-text("Add Appointment"), a[href*="/appointments/Add"]';
        const trigger = await page.waitForSelector(triggerSelector, { timeout: 15000 });
        await trigger.click();
